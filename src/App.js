...
export default function App() {
  ...
  const handleButtonClick = () => {
    // Kullanıcı butona tıkladığında gameRunning durumunu tersine çevir
    setGameRunning(!gameRunning);
  };

  // Arka plan rengini belirlemek için bir değişken
  const backgroundColor = gameRunning ? "lightcoral" : "lightblue";

  // Button üzerinde görünecek metni belirlemek için bir değişken
  const buttonText = gameRunning ? "Pause" : "Play";

  return (
    <div style={{ backgroundColor }}>
      {/* Challenge'daki şartlara göre buton */}
